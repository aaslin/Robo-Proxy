...
public class LoginUtils {
	 ...
	 */
	public static CookieManager loginAppEngine(String loginUrl, String serviceUrl, String email, String password) throws IOException, AuthenticationException {
		...
		try {
			...
			if (localDevMode) {
				...
				loginUrl = loginUrl + "/_ah/login?continue=" + URLEncoder.encode(serviceUrl, "UTF-8") + "&auth=" + token;
				URL url = new URL(loginUrl);
	...
	public static CookieManager loginFormBasedJ2EE(String loginUrl, String username, String password) throws IOException, URISyntaxException {
		CookieHandler oldCookieHandler = CookieHandler.getDefault();
		try {
			CookieManager cookieManager = new CookieManager(null, CookiePolicy.ACCEPT_ALL);
			CookieHandler.setDefault(cookieManager);
